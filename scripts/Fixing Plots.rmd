...
unique(combined_tdr$date) #check for issues in date column #some cases of "816124"

combined_tdr_dates <- combined_tdr %>% 
  mutate(date=if_else(date=="816124", "8/16/24", date)) %>%  #fix the error found above
...
bed_tdr_avg <- combined_tdr %>% 
  ...
  summarise(
    ...
    sem_TDR=sem(TDR, na.rm=TRUE), .groups = "drop")
bed_tdr_avg
...
bed_tdr_avg %>%
  ...
  scale_color_viridis_d(option = "viridis")
ggsave("../output/MeanTDRperBed_Time.png", width = 12, height = 8, units = "in")
...
bed_tdr_avg %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust =1, size = 11), text=element_text(size=25)) +
  labs(x = "Date", y = "Average TDR", color = "Bed",
    title = "Average TDR for Beds A, B, C K over Time") +
  scale_color_viridis_d(option = "viridis")
