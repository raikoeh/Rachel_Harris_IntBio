...
combined_tdr_dates %>% filter(is.na(TDR))
#no measurements for Beds B,C,D,F,J on 8/20, 8/27 - Remove these dates so that the averages aren't thrown off
#some beds only have data for one position on some dates...
```
...
bed_means <- combined_tdr_dates %>% 
  filter(date != "2024-08-20") %>% 
  filter(date != "2024-08-27") %>% #remove these dates since don't have data for all beds (might throw off the averages)
  group_by(Bed) %>% 
...
bed_pos_means <- combined_tdr_dates %>% 
  filter(date != "2024-08-20") %>% 
  filter(date != "2024-08-27") %>% #remove these dates since don't have data for all beds (might throw off the averages)
...
#Average TDR of A, B, I, J over Time - Middle Position 
bed_tdr_avg %>%
  filter(Bed %in% c("A", "B", "I", "J")) %>% 
  filter(Position=="middle") %>% 
  ggplot(aes(x =date, y =mean_TDR, color = Bed, group = Bed)) +
  geom_line(size = 1) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust =1, size = 11), text=element_text(size=25)) +
  labs(x = "Date", y = "Average TDR", color = "Bed",
    title = "Average TDR for Beds A, B, I, J over Time- Middle") +
  scale_color_viridis_d(option = "viridis")
ggsave("../output/MeanTDRMiddleBeds_Time.png", width = 12, height = 8, units = "in")

#Average TDR of A, C, D, E over Time - End Position 
bed_tdr_avg %>%
  filter(Bed %in% c("A", "C", "D", "E")) %>% 
  filter(Position=="end") %>% 
  ggplot(aes(x =date, y =mean_TDR, color = Bed, group = Bed)) +
  geom_line(size = 1) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust =1, size = 11), text=element_text(size=25)) +
  labs(x = "Date", y = "Average TDR", color = "Bed",
    title = "Average TDR for Beds A, C, D, E over Time- End") +
  scale_color_viridis_d(option = "viridis")
ggsave("../output/MeanTDREndBeds_Time.png", width = 12, height = 8, units = "in")
```

```{r}

#fixing MetStationData
...
#filtering for summer 2024 dates
...
```{r}
#initial filtering of MetStation data, replace MetStationData.csv with MetStation_2024
library(readr)
Data <- read.csv("MetStationData.csv", header = TRUE, sep = ",", quote = "", stringsAsFactors = FALSE)
head(Data)

Data$X1 <- NULL
Data <- Data[, c("X.TIMESTAMP", "VWC_5cm_Avg")]
write.csv(Data, file = "MetStationData.csv", row.names = FALSE)

library(dplyr)

Data_VWC <- Data %>% 
  select(X.TIMESTAMP, contains("VWC"))


write.csv(Data_VWC, file = "VWC_Data.csv", row.names = FALSE)

Available_VWC <- Data_VWC %>% 
  filter(if_any(contains("VWC"), ~ !is.na(.))) %>% 
  pull(X.TIMESTAMP)

writeLines(as.character(Available_VWC), "Available_Timestamps.txt")

```


```{r}
setwd("C:/Users/rachelharris/Desktop/Intbio_work/Rachel_Harris_IntBio/input")
```
