...
```
```{r}
Merged_BioMass_Size <- Merged_BioMass_Size[!is.na(Merged_BioMass_Size$height.cm) & !is.na(Merged_BioMass_Size$total.biomass_g), ]
```
...
Merged_BioMass_Size %>% 
  ggplot(aes(x=height.cm, y=total.biomass_g)) +
  geom_point(width = 0.3,position = position_dodge(0.75)) + 
  facet_wrap(~ `Plant Type`, scales = "free_x") +
  theme_bw() +
